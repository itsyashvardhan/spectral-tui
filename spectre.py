...
class EventManager:
    def __init__(self):
        ...
        self.flavor_messages = [
            ...
            "Packet intercepted [TCP/80]",
            "Brute-force mitigation active",
            "Rootkit check: Clean",
            "Public IP hidden",
            "Proxy chain: 3 nodes",
            "Handshake captured",
...
def get_network_info():
    ...
    try:
        ...
        if result.returncode == 0:
            info["ssid"] = result.stdout.strip() or "Ethernet"
    except:
        pass
    try:
...
def ping_host(host="8.8.8.8"):
    try:
        ...
        result = subprocess.run(
            ["ping", flag, "1", host], capture_output=True, text=True, timeout=2
        )
        if result.returncode == 0:
    ...
    try:
        if IS_LINUX:
            with open("/proc/meminfo", "r") as f:
                ...
                used = total - avail
                return f"{used // 1073741824}G/{total // 1073741824}G", (
                    used * 100
                ) // total
        elif IS_MAC:
...
def get_open_ports():
    ...
    try:
        if IS_WINDOWS:
            ...
            if IS_LINUX:
                result = subprocess.run(
                    ["ss", "-tuln"], capture_output=True, text=True, timeout=2
                )
                for line in result.stdout.split("\n")[1:]:
                    if "LISTEN" in line:
                        parts = line.split()
...
def get_processes():
    ...
    try:
        if IS_WINDOWS:
            ...
            args = (
                ["ps", "-eo", "pid,comm,%cpu,etime", "--sort=-%cpu"]
                if IS_LINUX
                else ["ps", "-A", "-o", "pid,comm,%cpu,time"]
            )
            ...
            if IS_MAC:
                lines.sort(
                    ...
                    if len(x.split()) > 2 and x.split()[2].replace(".", "", 1).isdigit()
                    else 0,
                    reverse=True,
                )
    ...
    try:
        if IS_LINUX:
            ...
            if subnet:
                ...
                try:
                    result = subprocess.run(
                        ["arp-scan", "-l"], capture_output=True, text=True, timeout=10
                    )
                    for line in result.stdout.split("\n"):
                        parts = line.split("\t")
        ...
        if not devices:
            ...
            for line in result.stdout.split("\n"):
                ...
                if ip and mac and ":" in mac:
                    devices.append({"ip": ip, "mac": mac[:17], "vendor": "Unknown"})
...
class FileBrowser:
    ...
    def refresh(self):
        try:
            ...
            dirs = sorted(
                [d for d in entries if os.path.isdir(os.path.join(self.path, d))]
            )
            files = sorted(
...
class Terminal:
    ...
    def execute(self, cmd):
        ...
        if cmd_lower == "proc":
            ...
            for p in procs[:30]:
                self.output.append(
                    f"{p['pid']:<8} {p['name']:<16} {p['cpu']:<8} {p['time']}"
                )
        ...
        # cd command
            ...
            if force_interactive or base_cmd in interactive_tools:
                try:
                    ...
                    stdscr.nodelay(True)
                    self.output.append(
                        f"[Exited {base_cmd if not force_interactive else 'interactive mode'}]"
                    )
                except Exception as e:
                    self.output.append(f"[Error: {e}]")
            else:
...
class SpectreTUI:
    ...
    def draw_logo(self, y, x, h, w):
        ...
        self.safe_addstr(
            theme_y, x + 13, THEME_NAMES[self.theme_idx].upper(), self.CYAN
        )
    ...
    def draw_stats(self, y, x, h, w):
        ...
        stats = [
            ...
            (
                "MEM",
                f"{mem_str} ({mem_pct}%)",
                self.GREEN if mem_pct < 80 else self.RED,
            ),
            (
                "DISK",
                f"{disk_str} ({disk_pct}%)",
                self.GREEN if disk_pct < 90 else self.RED,
            ),
        ...
        if cpu > 80:
            EVENTS.add(f"Cpu Spike Detected: {cpu}%", "WARN")
        if mem_pct > 85:
            EVENTS.add("Memory Critical", "WARN")
    ...
    def draw_time(self, y, x, h, w):
        ...
        self.safe_addstr(
            cy + 2, x + (w - len(now.strftime("%A"))) // 2, now.strftime("%A"), self.DIM
        )
    ...
    def draw_terminal(self, y, x, h, w):
        ...
        input_y = y + h - 2
        self.safe_addstr(
            input_y, x + 2, f"{prompt}{self.terminal.input}"[: w - 4], self.GREEN
        )
    ...
    def handle_key(self, key):
        if (key == ord("q") or key == ord("Q")) and self.active_pane != 1:
            self.running = False
        elif key == 20:  # Ctrl+T
            self.cycle_theme()
        elif key == 9:  # TAB
...
def loading(stdscr):
    ...
    for i, stage in enumerate(stages):
        ...
        try:
            stdscr.addstr(
                h // 2, (w - bar_w - 10) // 2, f"[{bar}] {int(prog * 100):3d}%", GREEN
            )
        except:
            pass

        try:
            stdscr.addstr(h // 2 + 2, (w - len(stage)) // 2, stage, GREEN)
        except:
            pass
